  ...
  return (
    <header id="story" onScroll={onScrollUpdate}>
      <div
        ...
        className={`menu-btn ${isHambChanged ? 'change' : ''} ${pass50 ? '' : 'hide'}`}
      >
      ...
      <nav className={`fixed-nav ${isNavActive ? 'active' : ''}`}>
        <ul>
          {Menu.map((item, key) => (
            <li
              key={key}
              style={{ transitionDelay: `calc(.5s + (.3s*${key}))` }}
            >
              <a href={`https://adrianz-portfolio.netlify.app${item.link}`}>{item.name}</a>
            </li>
          ))}
        </ul>
      </nav>
